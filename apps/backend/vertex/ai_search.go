...
func servingConfig(projectID string,
	location string,
	collection string,
	ds DATASTORE) string {
...
func (s *VertexAISearch) GetHalfReview(employeeId string) (schema.PFSkillSearchModelsHalfReview, error) {
	...
	servingConfig := servingConfig(os.Getenv("GOOGLE_CLOUD_PROJECT"), "us", "default_collection", DS_HALF_REVIEW)
	if err != nil {
		log.Printf("Failed to create discovery engine client: %v", err)
...
func (s *VertexAISearch) GetMonthlyReview(employeeId string) (schema.PFSkillSearchModelsMonthlyReview, error) {
	...
	servingConfig := servingConfig(os.Getenv("GOOGLE_CLOUD_PROJECT"), "us", "default_collection", DS_MONTHLY_REVIEW)
	if err != nil {
		log.Printf("Failed to create discovery engine client: %v", err)
